...
def process_csv(input_filename):
    ...
    df_rgb_resampled = df[["R", "R_filter", "G", "G_filter", "B", "B_filter", "AC_red", "DC_red", "AC_green", "DC_green", "AC_blue", "DC_blue", "RR", "POS_AC", "CHROM_AC", "RR_filter",  "Pridict_SPO2"]].resample('1s').mean().reset_index()
    # 刪除 'Time' 欄位
    df_rgb_resampled.drop(columns=['Time'], inplace=True)

    # 四捨五入到整數
    df_rgb_resampled['Pridict_SPO2'] = df_rgb_resampled['Pridict_SPO2'].round()
